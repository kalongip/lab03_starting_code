...
function Channel(elementContext) {
    ...
    this.updateVisualizationData = function updateVisualizationData() {
        ...
        if(length > this.canvasReference.width) {
            ...
            for(var i = 0; i < this.canvasReference.width; ++i) {
                var dataFrom = Math.round(i * dataPerPixel);
                var dataTo = Math.round((i + 1) * dataPerPixel) + 1;
            ...
            for(var i = 0; i <= length; ++i) {
                ...
                if(i < 0 || i >= data.length) {
                    ...
                    this.visualizationData.push({ x: x, y : data[i] });
                }
                x += pixelPerData;
            }
    ...
    this.paintOverlayMarkings = function paintOverlayMarkings(canvasContext) {
        ...
        // The 0 seconds
        canvasContext.fillText("0s", 2, halfHeight + textOffset);

        // The `duration`
        canvasContext.textAlign = "right";
        var t = this.toDP(this.getSampleToSeconds(this.viewResolution), 4);
        canvasContext.fillText(t + "s", width, halfHeight + textOffset);

        // The half of `duration`
        canvasContext.textAlign = "center";
        t = this.toDP(this.getSampleToSeconds(this.viewResolution / 2), 4);
    ...
    this.zoom = function zoom(numberOfSamples) {
    ...
    this.generateWaveform = function generateWaveform() {
        ...
        console.log("Generating waveform of type '" + selectedWaveType + "' with frequency " + freq + "Hz and amplitude " + amp);
    ...
    this.generateMusic = function generateMusic(data) {
        ...
        var enabledPostProcessing = $("#enablePostProcessing").is(":checked");
        ...
        var useDuration = $("#useDuration").is(":checked");
        ...
        for(var i = 0; i < data.length; ++i) {
            ...
            newDuration += useDuration ? data[i].duration : 6.0;
            maxDuration = Math.max(newDuration, maxDuration);
        }
        maxDuration = Math.round(maxDuration) * sampleRate;
        ...
        for(var i = 0; i < data.length; ++i) {
            ...
            if(useDuration) {
                newWaveformAudioSequence = WaveformGenerator.generateWaveform(selectedWaveType, freq, noteAmp, data[i].duration);
            } else {
