...
function audioController(elementContext) {
    ...
    this.zoomToCycles = function zoomToCycles(numberOfCycles) {
        for(var i = 0; i < this.audioController.listOfChannels.length; ++i) {
            this.audioController.listOfChannels[i].zoomToCycles(numberOfCycles);
        }
    };
    ...
    this.zoomToSeconds = function zoomToSeconds(numberOfSeconds) {
        for(var i = 0; i < this.audioController.listOfChannels.length; ++i) {
            this.audioController.listOfChannels[i].zoomToSeconds(numberOfSeconds);
        }
    };
    ...
    // Update the download music link
    this.updateDownloadMusicLink = function updateDownloadMusicLink(saveMusicLink) {
        var url = this.toWave().toBlobUrlAsync("application/octet-stream");
        $(saveMusicLink).attr("href", url);
        var fileName = "music-" + currentWaveformType + ".wav"
        $(saveMusicLink).attr("download", fileName);
    };
    ...
    this.generateMusic = function generateMusic(data) {
        ...
        if($("#autoAmplitude").is(":checked")) {
            Postprocessor.postprocess(this.audioController.listOfChannels, "boost", 0);
        }

        // Update the download link
        this.updateDownloadMusicLink("#saveMusicLink");
    ...
    this.elementContext.updateDownloadMusicLink = this.updateDownloadMusicLink;
    this.elementContext.toWave = this.toWave;
    this.elementContext.generateWaveform = this.generateWaveform;
    this.elementContext.generateMusic = this.generateMusic;
